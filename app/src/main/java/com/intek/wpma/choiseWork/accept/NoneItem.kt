...
class NoneItem : Search() {
    ...
    override fun refreshActivity() {
        ...
        if (noneAccItemLocal.isNotEmpty()) {
            ...
            for (DR in noneAccItemLocal) {
                ...
                koEf.text = ss.helper.byeTheNull(DR["CoefView"].toString()) //обрежем нули и точку
    ...
    private fun reactionKeyLocal(keyCode: Int):Boolean {
        ...
        if (ss.helper.whatDirection(keyCode) == "Down") {
            if (currentLine < noneAccItemLocal.count()) currentLine++ else currentLine = 1
        } else {
            if (currentLine > 1) currentLine-- else currentLine = noneAccItemLocal.count()
            }
