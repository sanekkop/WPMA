...
class ItemCard : Search() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        countMarkPac = if (intent.extras!!.getString("countMarkPac").isNullOrEmpty()) 0
                        else intent.extras!!.getString("countMarkPac").toString().toInt()
        countMarkUnit = if (intent.extras!!.getString("countMarkUnit").isNullOrEmpty()) 0
                        else intent.extras!!.getString("countMarkUnit").toString().toInt()
        countItemAcc = if (intent.extras!!.getString("countItemAcc").isNullOrEmpty()) 0
                        else intent.extras!!.getString("countItemAcc").toString().toInt()
    ...
    private fun refreshActiveElement() {
    ...
    override fun reactionKey(keyCode: Int, event: KeyEvent?): Boolean {
        ...
        if (keyCode == 67) {
            //это Delete, обнулим значение если это коэффициент
            isMoveButton = true
    ...
    private fun completeAccept(): Boolean {
        ...
        if (checkMark() == 0) {
            ...
            if (ss.excStr == "") FExcStr.text = "Маркированный товар! Сканируйте маркировку!"
            else FExcStr.text = ss.excStr
            return false
        }
       ...
       if (!saveUnitItem()) return false
        ...
        if (tmpCoef == 0) tmpCoef = currentRowAcceptedItem["Coef"].toString().toInt()
    ...
    private fun saveUnitItem() :Boolean {
        ...
        for (dr in fUnits) {
            ...
            if (dr["ID"].toString() != ss.getVoidID()) {
                ...
                if (dr["Barcode"].toString().trim() != "" || !findDr) needNew++
            }
        ...
        if (needNew > 0) {
            ...
            for (dr in fUnits) {
                if (dr["Coef"].toString().toInt() == 0) continue
                if (dr["ID"].toString() == ss.getVoidID()) {
                    var findDr = false
                    if (tmpDT.isNotEmpty()) {
                        for (tmpdr in tmpDT) {
                    ...
                    if (dr["Barcode"].toString().trim() == "" && findDr) continue
                    if (!createUnit(
                            dr["OKEI"].toString(),
