...
class UnLoading : BarcodeDataReceiver() {
    ...
    private fun reactionBarcode(Barcode: String): Boolean {
        ...
        if (typeBarcode == "113") {
            ...
            if (ss.isSC(idd, "Сотрудники")) {
                ...
                if (currentAction != Global.ActionSet.ScanAddress) {
                ...
                if (doc == null) {
                    FExcStr.text =
                        ("Неверно! " + if (currentAction == Global.ActionSet.ScanAddress) "Отсканируйте адрес." else "Отсканируйте коробку.")
                    badVoice()
                    return false
                }
                ...
                if (docUnload.typeDoc == "КонтрольНабора") {
                    ...
                    if (dt == null) {
                        FExcStr.text =
                            ("Ошибка запроса! " + if (currentAction == Global.ActionSet.ScanAddress) "Отсканируйте адрес." else "Отсканируйте коробку.")
                        return false
                    }

                    if (dt.isEmpty()) {
                        FExcStr.text =
                            ("Не найдено место! " + if (currentAction == Global.ActionSet.ScanAddress) "Отсканируйте адрес." else "Отсканируйте коробку.")
                        badVoice()
                        return false
                    }
                    boxUnLoad = dt[0]["ID"].toString()
                    currentAction = Global.ActionSet.ScanAddress
                    adressUnLoad = ""
                } else {
                    FExcStr.text =
                        ("Неверно! " + if (currentAction == Global.ActionSet.ScanAddress) "Отсканируйте адрес." else "Отсканируйте коробку.")
            ...
            FExcStr.text =
                ("Нет действий с данным ШК! " + if (currentAction == Global.ActionSet.ScanAddress) "Отсканируйте адрес." else "Отсканируйте коробку.")
            badVoice()
            return false
        }
        goodVoice()
    ...
    private fun refreshActivity() {
        ...
        FExcStr.text =
            if (currentAction == Global.ActionSet.ScanAddress) "Отсканируйте адрес." else "Отсканируйте коробку."
